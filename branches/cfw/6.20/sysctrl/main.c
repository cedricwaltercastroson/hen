...
static int sceIdStorageLookup_USB(u16 key, u32 offset, void *buf, u32 len);
...
static void PatchUSB(u32);
static void PatchRegistry(u32);
...
int (*g_sceIdStorageLookup)(u16, u32, void *, u32) = NULL;
int (*g_sceRegOpenRegistry)(struct RegParam *reg, int mode, REGHANDLE *h) = NULL;
int (*g_sceRegCloseRegistry)(REGHANDLE) = NULL;
int (*g_sceRegOpenCategory)(REGHANDLE, const char *, int, REGHANDLE*) = NULL;
int (*g_sceRegCloseCategory)(REGHANDLE) = NULL;
int (*g_sceRegGetKeyValueByName)(REGHANDLE, const char *, void*, SceSize) = NULL;
		...
		int mode, void *unk1, long unk2)
{
	...
	if (mode == IOASSIGN_RDWR) {
		if (!strcmp(dev3, "flashfat0:") && config.flags & CF_FLASHPROTECT) {
			pspSdkSetK1(k1);
			return -1;
		}
	}
...
#define __panic() do { *(int *) 0 = 0; } while (1)
	...
	} else if (!strcmp(mod->modname, "sceRegistry_Service")) {
		PatchRegistry(text_addr);
	} else if (!strcmp(mod->modname, "sceUSB_Driver")) {
		PatchUSB(text_addr);
...
{
	ASM_FUNC_TAG();
	if (!(config.flags & CF_SKIPGAMEBOOT))
...
PatchGamePlugin(u32 text_addr)
{
	...
	if (config.flags & CF_HIDEPICTURE) {
...
int ReadRegistry(const char *dir, const char *name, void *val, int len)
{
	...
	if(g_sceRegOpenRegistry(&reg, 1, &rh) < 0) return 0;
	if(g_sceRegOpenCategory(rh, dir, 1, &rc) < 0) {
		g_sceRegCloseRegistry(rh);
		return 0;
	}
	if(g_sceRegGetKeyValueByName(rc, name, val, len) < 0) {
		g_sceRegCloseCategory(rc);
		g_sceRegCloseRegistry(rh);
		return 0;
	}
	g_sceRegCloseCategory(rc);
...
int UTF8toUNICODE(char *utf8, wchar_t *unicode, int limit)
{
	unsigned char *ptr = (unsigned char *)utf8;
	int len = 0;

	while(*ptr) {
		if(*ptr & 0x80) { // maybe UTF-8 char
			if((*ptr & 0xF0) == 0xE0) { // UTF-8 (0x0800 - 0xFFFF)
				*unicode = *ptr << 12;
				ptr++;
				if(*ptr == 0) break;
				if((*ptr & 0xC0) != 0x80) continue; // next is not UTF-8, ignore
				*unicode += (*ptr & 0x3F) << 6;
				ptr++;
				if(*ptr == 0) break;
				if((*ptr & 0xC0) != 0x80) continue; // next is not UTF-8, ignore
				*unicode += *ptr & 0x3F;
			} else if((*ptr & 0xE0) == 0xC0) { // UTF-8 (0x0080 - 0x07FF)
				*unicode = (*ptr & 0x1F) << 6;
				ptr++;
				if(*ptr == 0) break;
				if((*ptr & 0xC0) != 0x80) continue; // next is not UTF-8, ignore
				*unicode += *ptr & 0x3F;
			} else { // unsupported character, ignore
				ptr++;
				continue;
			}
		} else *unicode = *ptr; // ASCII (0x0000 - 0x007F)
		ptr++;
		unicode++;
		len++;
		if(len == limit) break;
	}
//	*unicode = *ptr; // zero last char
	return len;
}

unsigned char key41buff[512];

int sceIdStorageLookup_USB(u16 key, u32 offset, void *buf, u32 len)
{
	static int prepared = 0;
	static wchar_t nameend[] = L"'s PSP";
	char owner_name[32];
	int ret;

	ASM_FUNC_TAG();

	if(key == 0x0041 && config.flags & CF_USBNAME && prepared != 2) {
		if(!prepared) {
			if(g_sceIdStorageLookup(key, 0, key41buff, 512) >= 0 && ReadRegistry("/CONFIG/SYSTEM", "owner_name", owner_name, 32)) {
				prepared = 1;
				ret = UTF8toUNICODE(owner_name, (wchar_t*)&key41buff[0x4E], 25);
				memcpy(key41buff + 0x4E + (ret * 2), nameend, sizeof(nameend) - 2);
				key41buff[0x4C] = (ret * 2) + sizeof(nameend);
			} else prepared = 2;
		}
		if(prepared == 1) {
			memcpy(buf, key41buff + offset, len);
			return 0;
		}
	}
	return g_sceIdStorageLookup(key, offset, buf, len);
}

void PatchUSB(u32 text_addr)
{
	ASM_FUNC_TAG();

	_sw(MAKE_JMP(sceIdStorageLookup_USB), text_addr + 0x8FC8);
	_sw(0, text_addr + 0x8FCC);
	g_sceIdStorageLookup = (void*)sctrlHENFindFunction("sceIdStorage_Service", "sceIdStorage_driver", 0x6FE062D1);
}

void PatchRegistry(u32 text_addr)
{
	ASM_FUNC_TAG();

	g_sceRegOpenRegistry = (void*)(text_addr + 0x00000258);
	g_sceRegCloseRegistry = (void*)(text_addr + 0x000003D8);
	g_sceRegOpenCategory = (void*)(text_addr + 0x00001D84);
	g_sceRegCloseCategory = (void*)(text_addr + 0x00002438);
	g_sceRegGetKeyValueByName = (void*)(text_addr + 0x00004BCC);
}
...
{
	ASM_FUNC_TAG();
	static wchar_t g_verinfo[] = L"6.20? CFW";
	static wchar_t g_macinfo[] = L"hidden";

	int ver = sctrlHENGetVersion();

	g_verinfo[4] = (ver & 0xF) + 0x61;
	...
	if (config.flags & CF_HIDEMAC) {
		memcpy((void *) (text_addr + 0x0002DB90), g_macinfo, sizeof(g_macinfo));
	}

	if (g_model == 0 && config.flags & CF_SLIMCOLOR) {
...
sceCtrlReadBufferPositive_Patched(SceCtrlData *pad_data, int count)
{
	...
	cpuspeed = config.vshcpuspeed;
	busspeed = config.vshbusspeed;
...
*/
	...
	if (cpuspeed != 0 && busspeed != 0 && g_scePowerGetCpuClockFrequency() != cpuspeed) {
	...
	if (g_satelite_mod_id != -1) {
		...
		if (config.flags & CF_FASTSCROLL)
			fastscroll(pad_data, count);

		if(!(config.flags & CF_USEVSHMENU))
			goto out;
		...
		modid = sceKernelLoadModule("flash0:/vsh/module/vshmenu.prx", 0, NULL);
		if (modid >= 0) {
...
PatchSceImposeDriver(void)
{
	...
	sctrlSEGetConfig(&tconf);
	if(tconf.magic != CF_MAGIC || tconf.version != CF_VERSION) {
		// broken config, save defaults
		sctrlSESetConfig(&config);
	} else memcpy(&config, &tconf, sizeof(CFWconf));

	PatchRegion();
	if (g_model == 0)
		goto out;
	if (config.flags & CF_USBCHARGE)
		goto out;
...
void LoadPlugins(char *filename)
{
	...
	sceKernelAllocateFpl(fpl, (void **) &buff, NULL);
	if(buff) {
		len = sceIoRead(fd, buff, 0x400);
		name = ptr = buff;
		end = buff + len;
		sceIoClose(fd);
		line = 0;
		...
		sceKernelFreeFpl(fpl, buff);
	}
	sceKernelDeleteFpl(fpl);
}

void LoadFlashPlugins(int flag)
{
	int i;
	char name[48] = "flash0:/plugins/";

	for(i = 0; i < MAX_PLUGINS; i++) {
		if(config.plugins[i].flags & flag) {
			memcpy(name + 16, config.plugins[i].name, 32);
			StartPlugin(name);
		}
	}
}
...
sceKernelStartModule_Patched(int modid, SceSize argsize, void *argp, int *modstatus, SceKernelSMOption *opt)
{
	...
	switch (sceKernelApplicationType()) {
		case PSP_INIT_KEYCONFIG_VSH:
			LoadFlashPlugins(PF_VSH);
			LoadPlugins("ms0:/seplugins/vsh.txt");
			if(g_model == 4) LoadPlugins("ef0:/seplugins/vsh.txt");
		break;
		case PSP_INIT_KEYCONFIG_GAME:
			LoadFlashPlugins(PF_GAME);
			LoadPlugins("ms0:/seplugins/game.txt");
			if(g_model == 4) LoadPlugins("ef0:/seplugins/game.txt");
		break;
	}
/*
	if ((fpl = sceKernelCreateFpl("", 1, 0, 0x400, 1, NULL)) < 0)
