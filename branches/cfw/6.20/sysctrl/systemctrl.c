...
sctrlSEGetConfigEx(void *buf, SceSize size)
{
	...
	k1 = pspSdkSetK1(0);
	memset (buf, 0, size);
	if ((fd = sceIoOpen("flashfat1:/kgCFW.cfg", PSP_O_RDONLY, 0777)) > 0) {
...
{
	ASM_FUNC_TAG();
	return sctrlSEGetConfigEx(buf, sizeof(CFWconf));
}

/* 0x000031A4 */
/* SystemCtrlForUser_AD4D5EA5 SystemCtrlForKernel_AD4D5EA5 */
int
sctrlSESetConfigEx(CFWconf *config, int size)
{
	ASM_FUNC_TAG();
	int ret = 0;
	u32 k1;
	...
	k1 = pspSdkSetK1(0);
	if((fd = sceIoOpen("flashfat1:/kgCFW.cfg", PSP_O_WRONLY | PSP_O_CREAT | PSP_O_TRUNC, 0777)) < 0) {
		pspSdkSetK1(k1);
		return -1;
	}
...
int
sctrlSESetConfig(CFWconf *config)
{
	ASM_FUNC_TAG();
	return sctrlSESetConfigEx(config, sizeof(CFWconf));
}
...
void
SetConfig(CFWconf *config)
{
	ASM_FUNC_TAG();
	memcpy(&config, config, sizeof(CFWconf));
}
...
/* VshCtrlLib_CD6B3913 */
int
vctrlVSHExitVSHMenu(CFWconf *conf)
{
	ASM_FUNC_TAG();
	int k1 = pspSdkSetK1(0);
	int cpuspeed = config.vshcpuspeed;
	int busspeed = config.vshbusspeed;

	VshMenuCtrl = NULL;
	memcpy(&config, conf, sizeof(CFWconf));

	if (g_timestamp_2 == 0) {
		if (cpuspeed != config.vshcpuspeed) {
