...
static void parseconfig(CFWconf *); /* 0x000002F8 */
...
CFWconf *g_config = NULL; /* 0x00001DF4 */
...
{
	static CFWconf config = {0}; /* 0x00001E94 */
...
parseconfig(CFWconf *config)
{
	...
	if (cpuspeed_index(config->gamecpuspeed) == 0 ||
			busspeed_index(config->gamebusspeed) == 0) {
		scePaf_sprintf(isospeed, "Default");
	} else {
		scePaf_sprintf(isospeed, "%d/%d", config->gamecpuspeed, config->gamebusspeed);
	}
	g_menu[1] = isospeed;

	g_menu[2] = g_regions[config->fakeregion];
	g_menu[3] = g_choices[!(config->flags & CF_SKIPGAMEBOOT)];
	g_menu[4] = g_choices[!(config->flags & CF_HIDEMAC)];
	g_menu[5] = g_choices[!(config->flags & CF_USEVSHMENU)];
	g_menu[6] = g_choices[!(config->flags & CF_HIDEPICTURE)];
	g_menu[7] = g_choices[!(config->flags & CF_USBNAME)];
	g_menu[8] = g_choices[!(config->flags & CF_USBCHARGE)]; // also CF_SLIMCOLOR
	g_menu[9] = g_choices[!(config->flags & CF_FASTSCROLL)];
	g_menu[10] = g_choices[!(config->flags & CF_FLASHPROTECT)];
}
...
#define update_busspeed(__spd, __off) do {\
	*__spd = g_bus_speeds[normalize(busspeed_index(*(__spd)) + (__off), 0, 8)];\
	...
	case 1:
		if (r != 0) {
			update_cpuspeed(&g_config->gamecpuspeed, r);
			update_busspeed(&g_config->gamebusspeed, r);
		}
		return 1;
	...
	case 3:
		if (r != 0)
			g_config->flags ^= CF_SKIPGAMEBOOT;
		return 1;

	case 4:
		if (r != 0)
			g_config->flags ^= CF_HIDEMAC;
		return 1;

	case 5:
		if (r != 0)
			g_config->flags ^= CF_USEVSHMENU;
		return 1;

	case 6:
		if (r != 0)
			g_config->flags ^= CF_HIDEPICTURE;
		return 1;

	case 7:
		if (r != 0)
			g_config->flags ^= CF_USBNAME;
		return 1;

	case 8:
		if (r != 0)
			g_config->flags ^= CF_USBCHARGE; // also CF_SLIMCOLOR
		return 1;

	case 9:
		if (r != 0)
			g_config->flags ^= CF_FASTSCROLL;
		return 1;

	case 10:
		if (r != 0)
			g_config->flags ^= CF_FLASHPROTECT;
		return 1;
...
draw_menu(void)
{
	...
	set_color(0x0000FFFF, 0x80FF0000);
	draw_string(0xC0, 0x30, "6.20a VSH menu");

	for (i = 0; i < 0x10; i++) {
		color = g_cur_index == i ? 0x000000FF : 0xC08080FF;
