...
u32
sub_0002124C(u32 a0, int *s1, u32 len, int *s2)
{
	static u32 (*func) (u32, int *, u32, int *) = NULL;
	u32 ret = 0;
	u32 *table;
...
static int sub_00046CBC_is_called = 0;
...
sub_00046CBC(u32 a0, u32 a1)
{
	...
	//logint(v0);
	logint(_lw(0x18+a0));
	logint(_lw(g_text_addr + 0x0005E68C));
	p = (int *) _lw(0x20 + a0);
	logint(p[0]);
	logint(p[1]);
	logint(p[2]);
...
u32
sub_00049794(u32 a0, u32 a1, u32 a2)
{
	static u32 (*func) (u32, u32, u32) = NULL;
	u32 ret;
	int *p;
	...
	ret = 0;
	load_text_addr(func, 0x00049794, ret);
	logstr("sub_00049794:");
	logint(a2);
	p = (int *) a1;
	for (i = 0; i < a2/4; i++) {
...
u32
sub_0004A304(u32 a0, u32 a1, u32 a2, u32 a3, u32 t0)
{
	static u32 (*func) (u32, u32, u32, u32, u32) = NULL;
	//u32 myra;
	u32 ret = 0;
	...
	//__asm__ volatile ("addiu %0, $ra, 0;" : "=r"(myra));
	load_text_addr(func, 0x0004A304, ret);
	if (sub_00046CBC_is_called)
		logstr("sub_0004A304:");
	//logint(myra);
	if (sub_00046CBC_is_called) {
...
sub_00009774(u32 a0)
{
	...
	__asm__ volatile ("addiu %0, $ra, 0;" : "=r"(myra));
	ret = 0;
	load_text_addr(func, 0x00009774, ret);
	logint(myra-g_text_addr);
	//logstr("sub_00009774:");
	ret = func(a0);
	//logstr("0x00009774:");
	logint(ret);
...
module_start(SceSize args, void* argp)
{
	...
	//jigsaw sctrlPatchModule("sceVshLftvMw_Module", MAKE_CALL(sub_00014C34), 0x000112F0);
